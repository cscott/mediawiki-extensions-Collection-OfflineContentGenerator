...
JobDetails.prototype.updatePending = function updatePending(status, progress) {
	this.update('pending', status, parseFloat(progress), null, null);
};

JobDetails.prototype.updateBundling = function updateBundling(article, status, progress) {
	this.update('progress', status, 0.5 * parseFloat(progress), article, null);
};

JobDetails.prototype.updateRendering = function updateRendering(page, status, progress) {
	this.update('progress', status, 50 + (0.5 * parseFloat(progress)), null, page);
};

JobDetails.prototype.updateFinished = function updateFinished(url, type, name, length) {
	this.update('finished', null, 100, null, null);
...
JobDetails.prototype.toJson = function toJson(jd) {
	return JSON.stringify(js);
};

exports.fromJson = function fromJson(jd) {
	var func, obj = JSON.parse(jd);

	for (func in JobDetails.prototype) {
		obj[func] = JobDetails.prototype[func];
	}
	for (func in JobStatus.prototype) {
