...
function newItemFromQueue( obj ) {
	...
	return Promise.promisify( fs.writeFile, false, fs )( metabookFile, JSON.stringify( jobDetails.metabook ) )
		.then( function() {
		...
		.catch( function( err ) {
			...
			console.error( err.message || err, {
				channel: channel,
				job: { id: jobDetails.collectionId, writer: jobDetails.writer,
					   // stringify metabook so winston doesn't break this into
					   // individual fields!
...
var fork = function( bin, args, options ) {
	return new Promise( function( resolve, reject ) {
		...
		child.on( 'exit', function( code, signal ) {
			...
			e.code = code;
			e.signal = signal;
			e.timeout = ( signal === 'SIGTERM' ) && killed;
			return reject( e );
		});
...
function runRenderer( jobDetails, renderTempDir, bundleFile ) {
	...
	return redisClient.hset(
		...
		return Promise.promisify( fs.stat, false, fs )( renderFile ).
			catch(function(err) {
				throw new BackendError( util.format('Could not stat rendered output file. Render failed!', renderFile, err ) );
			});
	}).then(function(stat) {
