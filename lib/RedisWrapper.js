 ...
 *
 * @param retryMaxDelay Max delay between connection attempts.
 */
RedisWrapper.prototype.connect = function(retryMaxDelay) {
	var self = this;
	...
	this.client = redis.createClient(
		...
		this.host,
		{
			...
			retry_max_delay: retryMaxDelay || 60000,
...
RedisWrapper.prototype.hscan = function(key, cursor) {
	...
	return Promise.promisify(this.client.hscan, false, this.client)(key, cursor).catch(function(err) {
		// The (stable) redis 2.6 doesn't support hscan, boo.
		...
		var hkeys = Promise.promisify(this.client.hkeys, false, this.client);
		var hmget = Promise.promisify(this.client.hmget, false, this.client);
		return hkeys(key).then(function(keys) {
			...
			var some = function() {
				...
				i += COUNT;
				return hmget(key, next).then(function(values) {
					var j, result = [];
					for (j = 0; j < next.length; j++) {
						result.push(next[j]);
						result.push(values[j]);
					}
					return [ i < keys.length ? some : 0, result ];
				});
			};
			return some();
		});
