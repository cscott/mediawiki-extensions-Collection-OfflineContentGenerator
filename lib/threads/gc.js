...
function clearJobStatusObjects() {
	...
	var iterateRedis = function( iterationResult ) {
		...
		var fjl = Date.now() - config.garbage_collection.failed_job_lifetime,
		...
		for ( var i = 0; i < iterationResult[1].length; i += 2 ) {
			...
			if ( job.status === 'failed' && ( job.timestamp < fjl ) ) {
				clearedFailedJobs += 1;
				multi.hdel( config.redis.status_set_name, job.collectionId );
		...
		if ( redisIterator === "0" ) {
			...
			return Promise.promisify( multi.exec, false, multi )()
				.then( function() {
					return multi.hscan( config.redis.status_set_name, redisIterator );
				} )
				.then( iterateRedis );
		}
	};
	...
	return redisClient
		...
		.then( function() {
			return redisClient.hscan( config.redis.status_set_name, redisIterator );
		} )
		.then( iterateRedis );
}
