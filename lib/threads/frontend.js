...
function handleRequest( requestId, args, response ) {
	...
	switch ( args.command ) {
		...
		default:
			throw new FrontendError( sprintf( 'Unrecognized command: %s', args.command ), 400 );
	}
}
...
function handleRender( requestId, args, response ) {
	...
	function determineCachedStatus() {
		var jobDetails;
			...
			return redisClient.hget( config.redis.status_set_name, collectionId )
				.then( function( result ) {
					if ( result ) {
						...
						return ( jobDetails.state !== "failed" );
					} else {
						return false;
					}
				} )
	...
	return Promise.resolve( determineCachedStatus() )
		.then( function( cacheStatus ) {
			isCached = cacheStatus;
			if ( !isCached || forceRender ) {
				return Promise.resolve( canIRender() )
 ...
 */
getFolderSize = Promise.method(function( fspath ) {
	...
	return lstat( fspath ).then( function( stat ) {
		...
		return readdir( fspath ).reduce(function(total, item) {
		...
		console.warn( "Could not find size of", fspath, ":", err );
		return 0;
	} );
});
