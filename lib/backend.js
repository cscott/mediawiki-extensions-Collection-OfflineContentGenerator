...
function runBundler(jobDetails, metabookPath) {
	var child;
	var bundle = path.join(config.get('backend:temp_dir'), jobDetails.collectionId + '.zip');

	// TODO: Don't do this if we have a cached bundle
	console.debug('Forking child to run job id %s, %s', jobDetails.collectionId, '-m ' + metabookPath);
