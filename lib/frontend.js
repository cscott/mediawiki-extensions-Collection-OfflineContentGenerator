...
function handleRequest(args, response) {
	console.log('attempting to handle request: %s', JSON.stringify(args));
	try {
		switch (args.command) {
			case "render":
...
/**
 * Handle the bookcmd=render command
 *
...
function handleRender(args, response) {
	...
	if (!language || (typeof language !== 'string')) {
		throw new FrontendError('Cannot begin "render" command: parameter "language" not specified', 400);
	}
	...
	if (!collectionId) {
		...
		if (!metabook) {
			...
			collectionId = createCollectionId(writer, metabookObj);
		...
		try {
			...
			redisClient.multi()
				...
				.hset(
					...
					collectionId,
					JSON.stringify(new jd.JobDetails(
						collectionId,
						metabookObj,
						writer,
						language
				...
				.exec(function(err, replies) { if (err) { throw err; } });
		} catch (err) {
		...
		redisClient.hget(
			...
			function(err, result) {
				if (err) {
					...
					obj = JSON.parse(result);
					if (obj.renderedFileLoc) {
						fs.stat(obj.renderedFileLoc, function (err, stats) {
							if (err) {
								console.debug('Could not stat file "%s" for job %s', obj.renderedFileLoc, collectionId);
								response.writeHead(404, 'Backend file could not be found');
							} else {
								console.debug('Starting stream of file %s', obj.renderedFileLoc);
								fs.writeHead(200, {'Content-Type': mime.getType(obj.renderedFileLoc)});
								fs.createReadStream(obj.renderedFileLoc ).pipe(response);
							}
						});
					}
				}
 ...
 * is rendered.
 *
 * @param writer string Type of renderer being used
