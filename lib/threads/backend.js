...
var rimraf = Promise.promisify((function(rimraf) {
	return function rmrf(filename, cb) {
		// Force the `disableGlob` option, for security.
		return rimraf(filename, { disableGlob: true }, cb);
	};
})(require('rimraf')));
var os = require('os');
 ...
 *
 * @param {hash} configObj - Configuration object
 */
function initBackend(configObj) {
	var writer, paths;

	config = initDirectories(configObj);
...
function newItemFromQueue(obj) {
	...
	return Promise.promisify(fs.writeFile, false, fs)(metabookFile, JSON.stringify(jobDetails.metabook, null, 2))
		...
		.catch(function(err) {
			...
			if (err instanceof BundlerError) { channel = 'backend.bundler.error'; } else if (err instanceof RendererError) { channel = 'backend.renderer.error'; }
...
var fork = function(bin, args, options) {
	return new Promise(function(resolve, reject) {
		...
		var child = childProcess.fork(bin, args, options.childOptions || {});
