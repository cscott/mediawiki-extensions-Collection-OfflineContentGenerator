...
function getNewItemFromQueue() {
	redisClient.blpop(config.get('redis:job_queue_name'), 0, function(err, obj) {
		if (err) {
			...
			if ((obj.length !== 2) || (!obj[1])) {
				...
				redisClient.hget(config.get('redis:status_set_name'), obj[1], newItemFromQueue);
			}
		}
	});
}
function newItemFromQueue(err, obj) {
	...
	if (err) {
		...
		tempMetabook = path.join(config.get('backend:temp_dir'), jobDetails.collectionId + '.json');
		fs.writeFile(
			tempMetabook,
...
function runBundler(jobDetails, metabookPath) {
	var child;
	var bundleFile = path.join(config.get('backend:temp_dir'), jobDetails.collectionId + '.zip');
	...
	child.on('error', function(err) {
		...
		redisClient.hset(config.get('redis:status_set_name'), jobDetails.collectionId, JSON.stringify(jobDetails));
		getNewItemFromQueue();
	});
	child.on('message', function(message, handle) {
		try {
			var msgObj = JSON.parse(message);
			jd.updateBundling(jobDetails, msgObj.file, msgObj.status, msgObj.percent);
			redisClient.hset(config.get('redis:status_set_name'), jobDetails.collectionId, JSON.stringify(jobDetails));
		} catch (err) {
			// Pass
		}
...
function runRenderer(jobDetails, bundleFile) {
	...
	var renderFile = path.join(
		...
		jobDetails.collectionId + config.get('backend:writers:' + writer + ':extension')
	);
	var url;
	...
	child = child_process.fork(config.get('backend:writers:' + writer + ':bin'), [
		'-o', renderFile,
		bundleFile
	]);
	child.on('error', function(err) {
	...
	child.on('message', function(message, handle) {
		try {
			var msgObj = JSON.parse(message);
			jd.updateRendering(jobDetails, msgObj.file, msgObj.status, msgObj.percent);
			redisClient.hset(config.get('redis:status_set_name'), jobDetails.collectionId, JSON.stringify(jobDetails));
		} catch (err) {
			// Pass
		}
	...
	child.on('exit', function(code, signal) {
		...
		if (code !== 0) {
			...
			url = 'http://' + (config.get('coordinator:hostname') || os.hostname()) +
